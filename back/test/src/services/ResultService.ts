...
export class ResultService {
  ...
  GetDataOfResult(result: string) {
    ...
    const time = parseInt(result.slice(result.indexOf(' '), result.indexOf('ms')));
    return { cm, time };
  }
  ...
  async GetResults({ testId }: { testId: string }) {
    try {
      ...
      for (let result of results) {
        ...
        if (defaultDistance - this.GetDataOfResult(result.data[result.data.length - 1]).cm >= 5) {
          approaches.push({
        ...
        allApproaches += approaches.length;
      }
      return { reports, allApproaches };
    } catch (err) {
      throw err;
    }
  }
