...
export class TestService {
  async CreateTest(data: {
    ...
    try {
      const { purchaseId, name, productId, reportingFrequency, identityLogin } = data;
      ...
      if (existingTest.length !== 0) {
        throw AppError.badRequest('You already have a test for the product with the device');
      }
  ...
  async ChangeTest(data: {
    ...
    try {
      ...
      const test = await TestModel.findById(testId);
      if (!test) {
        throw AppError.unauthorised('The test you are trying to edit does nor exist anymore!');
      }
      await this.CheckCompanyByPurchase(test.purchaseId, identity);
      await TestModel.findOneAndUpdate({ _id: testId }, { reportingFrequency, trackingRange });
      return { success: true };
    } catch (err) {
      throw err;
    }
  ...
  async CheckCompanyByPurchase(purchaseId: string, identityLogin: string) {
    try {
      ...
      if (!company.data.admins.includes(identityLogin) && company.data.owner !== identityLogin) {
